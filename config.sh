...
while true; do
    case $step in
        1)
            root_part=$(dialog --stdout --backtitle "$BT" --title "Root Partition" --inputbox "Enter the path for the / partition:" 10 60 "$root_part")
            exit_code=$?
            if [ $exit_code -ne 0 ]; then clear; echo "Configuration aborted."; exit 0; fi
            step=2
            ;;
        2)
            root_part_filesystem=$(dialog --stdout --backtitle "$BT" --title "Root Filesystem" --radiolist "Select the filesystem for /:" 15 50 5 \
            ...
            ;;
        3)
            separate_home_part=$(dialog --stdout --backtitle "$BT" --title "Home Partition" --inputbox "Enter the path for /home, or leave as 'none':" 10 60 "$separate_home_part")
            exit_code=$?
            if [ $exit_code -ne 0 ]; then step=2; continue; fi
            if [ "$separate_home_part" = "none" ]; then step=5; else step=4; fi
            ;;
        4)
            separate_home_part_filesystem=$(dialog --stdout --backtitle "$BT" --title "Home Filesystem" --radiolist "Select the filesystem for /home:" 15 50 5 \
            ...
            ;;
        5)
            separate_boot_part=$(dialog --stdout --backtitle "$BT" --title "Boot Partition" --inputbox "Enter the path for /boot, or leave as 'none':" 10 60 "$separate_boot_part")
            exit_code=$?
            ...
            ;;
        6)
            separate_boot_part_filesystem=$(dialog --stdout --backtitle "$BT" --title "Boot Filesystem" --radiolist "Select the filesystem for /boot:" 15 50 5 \
            ...
            ;;
        7)
            separate_var_part=$(dialog --stdout --backtitle "$BT" --title "Var Partition" --inputbox "Enter the path for /var, or leave as 'none':" 10 60 "$separate_var_part")
            exit_code=$?
            if [ $exit_code -ne 0 ]; then
            ...
            ;;
        8)
            separate_var_part_filesystem=$(dialog --stdout --backtitle "$BT" --title "Var Filesystem" --radiolist "Select the filesystem for /var:" 15 50 5 \
            ...
            ;;
        9)
            separate_tmp_part=$(dialog --stdout --backtitle "$BT" --title "Tmp Partition" --inputbox "Enter the path for /tmp, or leave as 'none':" 10 60 "$separate_tmp_part")
            exit_code=$?
            if [ $exit_code -ne 0 ]; then
            ...
            ;;
        10)
            separate_tmp_part_filesystem=$(dialog --stdout --backtitle "$BT" --title "Tmp Filesystem" --radiolist "Select the filesystem for /tmp:" 15 50 5 \
            ...
            ;;
        11)
            luks_encryption=$(dialog --stdout --backtitle "$BT" --title "Disk Encryption" --radiolist "Encrypt the system with LUKS?" 12 40 2 \
            ...
            ;;
        12)
            luks_passphrase=$(dialog --stdout --insecure --backtitle "$BT" --title "LUKS Passphrase" --passwordbox "Enter your encryption passphrase:" 10 50)
            exit_code=$?
            if [ $exit_code -ne 0 ]; then step=11; continue; fi
            if [ "$boot_mode" = "UEFI" ]; then step=13; else step=15; fi
            ;;
        13)
            efi_part=$(dialog --stdout --backtitle "$BT" --title "EFI Partition" --inputbox "Enter EFI partition path (e.g., /dev/sda1):" 10 60 "$efi_part")
            exit_code=$?
            if [ $exit_code -ne 0 ]; then
            ...
            ;;
        14)
            efi_part_mountpoint=$(dialog --stdout --backtitle "$BT" --title "EFI Mountpoint" --radiolist "Select EFI mountpoint:" 12 50 2 \
            ...
            ;;
        15)
            grub_disk=$(dialog --stdout --backtitle "$BT" --title "GRUB Disk" --inputbox "Enter disk for GRUB installation (e.g., /dev/sda):" 10 60 "$grub_disk")
            exit_code=$?
            if [ $exit_code -ne 0 ]; then
                if [ "$luks_encryption" = "yes" ]; then step=12; else step=11; fi
                continue
            ...
            ;;
        16)
            network_management=$(dialog --stdout --backtitle "$BT" --title "Network Management" --radiolist "Select a network manager tool:" 14 50 3 \
            ...
            ;;
        17)
            kernel_variant=$(dialog --stdout --backtitle "$BT" --title "Kernel Variant" --radiolist "Select kernel variant:" 14 50 3 \
            ...
            ;;
        18)
            mirror_location=$(dialog --stdout --backtitle "$BT" --title "Mirrors" --inputbox "Enter mirror country (comma-separated or 'none'):" 10 60 "$mirror_location")
            exit_code=$?
            if [ $exit_code -ne 0 ]; then step=17; continue; fi
            step=19
            ;;
        19)
            timezone=$(dialog --stdout --backtitle "$BT" --title "Timezone" --inputbox "Enter timezone (Region/City):" 10 60 "$timezone")
            exit_code=$?
            if [ $exit_code -ne 0 ]; then step=18; continue; fi
            step=20
            ;;
        20)
            hostname=$(dialog --stdout --backtitle "$BT" --title "Hostname" --inputbox "Enter machine hostname:" 10 60 "$hostname")
            exit_code=$?
            if [ $exit_code -ne 0 ]; then step=19; continue; fi
            step=21
            ;;
        21)
            username=$(dialog --stdout --backtitle "$BT" --title "Username" --inputbox "Enter the primary username:" 10 60 "$username")
            exit_code=$?
            if [ $exit_code -ne 0 ]; then step=20; continue; fi
            step=22
            ;;
        22)
            full_username=$(dialog --stdout --backtitle "$BT" --title "Full Name" --inputbox "Enter full name (optional, leave empty to skip):" 10 60 "$full_username")
            exit_code=$?
            if [ $exit_code -ne 0 ]; then step=21; continue; fi
            step=23
            ;;
        23)
            password=$(dialog --stdout --insecure --backtitle "$BT" --title "User Password" --passwordbox "Enter user password:" 10 60)
            exit_code=$?
            if [ $exit_code -ne 0 ]; then step=22; continue; fi
            step=24
            ;;
        24)
            language=$(dialog --stdout --backtitle "$BT" --title "System Language" --inputbox "Enter system language locale:" 10 60 "$language")
            exit_code=$?
            if [ $exit_code -ne 0 ]; then step=23; continue; fi
            step=25
            ;;
        25)
            tty_keyboard_layout=$(dialog --stdout --backtitle "$BT" --title "TTY Keyboard Layout" --inputbox "Enter TTY keyboard layout:" 10 60 "$tty_keyboard_layout")
            exit_code=$?
            if [ $exit_code -ne 0 ]; then step=24; continue; fi
            step=26
            ;;
        26)
            install_pipewire=$(dialog --stdout --backtitle "$BT" --title "Audio (PipeWire)" --radiolist "Install PipeWire for audio?" 12 40 2 \
            ...
            ;;
        27)
            gpu=$(dialog --stdout --backtitle "$BT" --title "GPU Driver" --radiolist "Select GPU driver:" 16 50 5 \
            ...
            ;;
        28)
            de=$(dialog --stdout --backtitle "$BT" --title "Desktop Environment" --radiolist "Select desktop environment:" 16 50 6 \
            ...
            ;;
        29)
            install_cups=$(dialog --stdout --backtitle "$BT" --title "Printing (CUPS)" --radiolist "Install CUPS printing support?" 12 40 2 \
            ...
            ;;
        30)
            create_swapfile=$(dialog --stdout --backtitle "$BT" --title "Swapfile" --radiolist "Create a swapfile?" 12 40 2 \
            ...
            ;;
        31)
            swapfile_size_gb=$(dialog --stdout --backtitle "$BT" --title "Swapfile Size" --inputbox "Enter swapfile size in GB:" 10 50 "$swapfile_size_gb")
            exit_code=$?
            if [ $exit_code -ne 0 ]; then step=30; continue; fi
            step=32
            ;;
        32)
            keep_config=$(dialog --stdout --backtitle "$BT" --title "Keep Copy of Config" --radiolist "Keep config.conf in user home folder after installation?" 12 40 2 \
